    ...
    <section id="contact" className="py-20 md:py-28 bg-background">
      <div className="container">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
          <h2 className="text-3xl md:text-4xl font-display font-bold text-foreground mb-4">{t("visit.title")}</h2>
          <p className="text-muted-foreground max-w-xl mx-auto">{t("visit.subtitle")}</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-5xl mx-auto">
          <div className="p-6 md:p-8 rounded-xl bg-card border border-border">
              ...
              <div className="flex gap-3">
                ...
                <div>
                  ...
                  <p className="text-secondary font-medium text-sm mt-1 flex items-center gap-1">
                    <Navigation className="w-3.5 h-3.5" /> Landmark: {main.landmark}
                  </p>
                </div>
              </div>
              <div className="flex gap-3 items-start">
                ...
                <div className="space-y-1">
                  {main.contact_numbers.map((num) => (
                    <a key={num} href={`tel:${num.replace(/\s/g, "")}`} className="block text-foreground text-sm hover:text-primary transition-colors">{num}</a>
                  ))}
                </div>
              </div>
              <div className="flex gap-3">
                ...
                <div>
                  <p className="text-foreground text-sm">{t("visit.monSat")}: {hours.monday_to_saturday}</p>
                  <p className="text-muted-foreground text-sm">{t("visit.sunday")}: {t("visit.closed")}</p>
                </div>
              </div>
            ...
            <a href={main.google_maps_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 h-12 px-6 mt-6 rounded-lg bg-primary text-primary-foreground font-semibold text-sm hover:opacity-90 transition min-h-[48px]">
          ...
          <div className="space-y-6">
            <div className="hidden lg:block rounded-xl overflow-hidden border border-border h-64">
              <iframe
                ...
                referrerPolicy="no-referrer-when-downgrade" title="Acureatus Clinic Location"
              />
            </div>
            <div className="p-6 rounded-xl bg-card border border-border">
              ...
              <a href={`tel:${secondary.contact_number?.replace(/\s/g, "")}`} className="text-sm text-primary font-medium mt-2 block">{secondary.contact_number}</a>
            </div>
          </div>
        </div>
      </div>
